...
class Room {
  ...
  stateFor(p) {
    return {
      ...
      choosingIcons: this.state === 'choosing' ? (p.id === this.drawerId && this.choices ? this.choices.map((c) => c.icon || '') : null) : null,
      wordIcon: (this.entry && this.state === 'drawing' && p.id === this.drawerId) ? (this.entry.icon || '') : null,
      canvas: this.ops,
  ...
  startTurn() {
    ...
    this.send(drawer, {
      ...
      icons: this.choices.map((c) => c.icon || ''),
      endsAt: this.endsAt,
      now: Date.now()
    });
  ...
  // Look up a unit by its description. Searches the admin notes and the tags, never the
  // word list wholesale, so a client cannot pull the answers out of it. Map entries are
  // excluded on purpose: this is for learning unit names.
  handleLookup(p, raw) {
    ...
    const q = String(raw == null ? '' : raw).replace(CTRL, '').trim().slice(0, 60);
    const terms = q.toLowerCase().split(/\s+/).filter(Boolean);
    if (!terms.length) return this.send(p, { t: 'lookup', q: q, results: [] });
    const out = [];
    for (const w of this.mgr.store.enabledWords()) {
    ...
    this.send(p, { t: 'lookup', q: q, results: out });
  }
