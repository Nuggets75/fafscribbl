...
const FACTION_TAGS = ['uef', 'cybran', 'aeon', 'seraphim', 'neutral', 'nomads'];
...
class Room {
  ...
  pool() {
    ...
    if (f.length) list = list.filter((w) => w.tags.some((t) => f.indexOf(t) !== -1));
...
class RoomManager {
  ...
  adminView() {
    return Array.from(this.rooms.values()).map((r) => ({
      ...
      state: r.state, round: r.round, rounds: r.settings.rounds,
