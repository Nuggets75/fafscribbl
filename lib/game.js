...
class Room {
  ...
  // Words are ANDed across groups and ORed inside one. A group with nothing picked is
  // ignored. A group's `always` tag (e.g. faction-less buildings) bypasses that group.
  pool() {
    ...
    for (const g of groups) {
      const sel = (tf[g.id] || []).filter((t) => g.tags.indexOf(t) !== -1);
      if (!sel.length) continue;
      list = list.filter((w) =>
        (g.always && w.tags.indexOf(g.always) !== -1) || w.tags.some((t) => sel.indexOf(t) !== -1));
    }
