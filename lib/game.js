...
const FACTION_TAGS = ['uef', 'cybran', 'aeon', 'seraphim', 'nomads'];
const KIND_TAGS = ['land', 'air', 'naval', 'structure', 'experimental'];
...
function sanitizeSettings(patch, base) {
  ...
  if ('factions' in patch) s.factions = (Array.isArray(patch.factions) ? patch.factions : []).filter(function (t) { return FACTION_TAGS.indexOf(t) !== -1; });
  if ('kinds' in patch) s.kinds = (Array.isArray(patch.kinds) ? patch.kinds : []).filter(function (t) { return KIND_TAGS.indexOf(t) !== -1; });
  return s;
}
...
class Room {
  constructor(mgr, code, settings) {
    ...
    this.settings = sanitizeSettings(settings, mgr.store.defaults());
  ...
  customEntries() {
    ...
    for (const p of parts) {
      ...
      seen.add(key);
      out.push({ id: 'c:' + key, word: p.slice(0, 48), hint: 'custom word', aliases: [], tags: ['custom'], custom: true });
    }
  ...
  pool() {
    ...
    let list = this.mgr.store.enabledWords();
    const f = this.settings.factions || [];
    const k = this.settings.kinds || [];
    // Faction-less words (mexes, pgens, radar, nuke, SMD ...) belong to every faction,
    // so a faction filter always keeps them rather than silently dropping them.
    if (f.length) list = list.filter((w) => w.tags.indexOf('neutral') !== -1 || w.tags.some((t) => f.indexOf(t) !== -1));
    if (k.length) list = list.filter((w) => w.tags.some((t) => k.indexOf(t) !== -1));
    if (!list.length) list = this.mgr.store.enabledWords();
  ...
  startTurn() {
    ...
    this.endsAt = Date.now() + this.settings.choiceTime * 1000;
    this.pushState();
    this.send(drawer, { t: 'choices', words: this.choices.map((c) => c.word), endsAt: this.endsAt, now: Date.now() });
  ...
  pick(byId, index, auto) {
    ...
    this.state = 'drawing';
    this.revealed = [];
    this.hintsLeft = this.settings.hints ? this.settings.hintCount : 0;
    this.turnStartedAt = Date.now();
    this.endsAt = this.settings.drawTime > 0 ? Date.now() + this.settings.drawTime * 1000 : 0;
    this.ops = [];
    this.undoMarks = [];
  ...
  scheduleHints() {
    ...
    const total = Math.max(1, this.settings.hintCount);
    const done = total - this.hintsLeft;
    let delay;
    if (this.settings.drawTime > 0) {
      const slice = (this.settings.drawTime * 1000) / (total + 1);
      delay = (this.turnStartedAt + slice * (done + 1)) - Date.now();
    } else {
      delay = 45000;
    }
  ...
  guessCorrect(p) {
    ...
    p.roundScore = pts;
    p.score += pts;
    this.send(p, { t: 'reveal', word: this.entry.word });
  ...
  canDraw(p) { return this.state === 'drawing' && p.id === this.drawerId; }
  ...
  endGame() {
    ...
    this.drawerId = null;
    this.entry = null;
    this.endsAt = Date.now() + GAME_END_MS;
...
module.exports = {
  ...
  CANVAS_W, CANVAS_H, FACTION_TAGS, KIND_TAGS, COLORS
};
