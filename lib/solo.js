...
class Solo {
  ...
  timeUp(s) { return this.endRound(s, false); }

  // Giving up on one drawing is the same as running out of time: no points, show the answer,
  // move on. It is only marked differently so the recap can say so.
  skip(s) { return this.endRound(s, true); }

  endRound(s, skipped) {
    if (s.done || !s.current) return { result: 'over' };
    const d = s.current;
    s.results.push({ word: d.word, icon: d.icon || '', points: 0, got: false, skipped: !!skipped });
    s.at++;
    s.current = null;
    return {
      result: 'over', points: 0, skipped: !!skipped,
      word: d.word, icon: d.icon || '', total: s.score
    };
  }
